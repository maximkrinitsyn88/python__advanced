...
def measure_me(nums: List[int]):
    ...
    time_func = end - start
    logger.info(f'Время потраченное на функцию: {time_func}')
    return results, time_func
...
if __name__ == "__main__":
    ...
    for it in range(15):
        ...
        result, time_func = measure_me(data_line)
        times.append(time_func.seconds)
        avg_time = sum(times) / len(times)
        print(f'Среднее время, затраченное на функцию measure_me: {avg_time} секунд')
